...
const BookingPage = () => {
  ...
  const bookingComplete = location.state?.bookingComplete;
  ...
  useEffect(() => {
    ...
    if (token && (user || userInfo)) {
      ...
      form.setFieldsValue({
        ...
        phone:
          userData.PhoneNumber || userData.phone || userData.phoneNumber || "",
    ...
    const fetchTimeSlots = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const availableSlots = timeSlotsData.filter(
            (slot) => !slot.isDeleted
          );
    ...
    const fetchDonationSchedule = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const uniqueDates = [
            ...new Set(scheduleData.map((item) => item.scheduleDate)),
          ];
  ...
  const handleFormSubmit = (values) => {
        ...
        content: (
          <div>
            <p style={{ marginBottom: "15px" }}>
              Để có thể đăng ký hiến máu, bạn cần hoàn thành đầy đủ thông tin cá
              nhân bao gồm:
            </p>
            ...
            <p
              style={{ marginTop: "15px", color: "#dc2626", fontWeight: "500" }}
            >
              Vui lòng cập nhật thông tin tại trang cá nhân trước khi đăng ký
              hiến máu.
            </p>
          </div>
        ),
    ...
    if (values.donationDate) {
      try {
        ...
        const matchingSchedule = donationSchedule.find(
          (schedule) =>
            schedule.scheduleDate &&
            schedule.scheduleDate.startsWith(selectedDateStr)
        );
          ...
          form.setFields([
            {
              name: "donationDate",
              errors: [
                "Ngày được chọn không khớp với lịch hiến máu. Vui lòng chọn lại",
              ],
            },
          ]);
          return;
  ...
  useEffect(() => {
    if (bookingComplete) {
      message.success({
        content:
          "Cảm ơn bạn đã đăng ký hiến máu! Chúng tôi sẽ liên hệ với bạn sớm nhất có thể.",
        duration: 8,
      });
    }
    ...
    if (location.state?.alreadyRegistered) {
      ...
      message.info({
        content:
          customMessage ||
          "Bạn đã có lịch hiến máu. Vui lòng kiểm tra thông tin hoặc chọn ngày khác.",
        duration: 6,
      });
    }
    ...
    <Layout>
          ...
          <div className="container">
            <div className="form-header">
              ...
              <Paragraph className="form-subtitle">
                Một hành động nhỏ, một sự sống lớn. Hãy cùng chúng tôi cứu sống
                những con người cần giúp đỡ.
              </Paragraph>
            </div>
                ...
                <div className="form-container">
                  ...
                  >
                    ...
                    <Row gutter={[24, 24]}>
                      ...
                      <Col xs={24} md={12}>
                        <Form.Item label="Số CMND/CCCD" name="nationalId">
                          <Input
                            className="form-input"
                      ...
                      <Col xs={24} md={12}>
                        <Form.Item
                          ...
                          rules={[
                            {
                              required: true,
                              message: "Vui lòng chọn ngày hiến máu",
                            },
                          ]}
                          normalize={(value) => {
                        ...
                        <Col xs={24}>
                          ...
                          >
                            <Text style={{ color: "#52c41a" }}>
                              ...
                              Thông tin cá nhân đã được điền tự động. Để thay
                              đổi thông tin, vui lòng cập nhật trong{" "}
                              <Button
                                type="link"
                                size="small"
      ...
      >
        <div className="login-modal-content">
          ...
          >
            Bạn cần đăng nhập để thực hiện đăng ký hiến máu. Thông tin đặt lịch
            của bạn sẽ được lưu lại.
          </Paragraph>
