...
const DonationSchedulePage = () => {
  ...
  const getCompletedDonationCount = () => {
    ...
    return registrations.filter(
      (registration) => registration.registrationStatusId === 3
    ).length;
  };
  ...
  const getLastCompletedDonationDate = () => {
    ...
    completedRegistrations.forEach((registration) => {
      ...
      if (scheduleId && donationSchedule && donationSchedule.length > 0) {
        const schedule = donationSchedule.find(
          (s) =>
            s.id === scheduleId ||
    ...
    try {
      ...
      if (!userInfo || !userId) {
        if (userInfo) {
          const fallbackData = {
            donationCount:
              userInfo.donationCount || userInfo.DonationCount || 0,
            lastDonationDate:
              userInfo.lastDonationDate || userInfo.LastDonationDate,
      ...
      if (response.status === 200 && response.data) {
        ...
        const newDonationInfo = {
          ...
          lastDonationDate:
            userData.lastDonationDate || userData.LastDonationDate,
          nextEligibleDonationDate:
    ...
    try {
      ...
      for (const registration of registrationsList) {
        if (registration.registrationStatusId === 3) {
          try {
            const response = await UserAPI.getFeedbackByRegistrationId(
            ...
            console.log(
              `Feedback check for registration ${registration.registrationId}: response =`,
              response.data,
              `exists = ${hasValidFeedback}`
            );
          } catch (error) {
            feedbackChecks[registration.registrationId] = false;
            console.log(
              `Feedback check for registration ${
                registration.registrationId
              }: error = ${error.response?.status || "unknown"}, exists = false`
            );
          }
        }
      }
  ...
  useEffect(() => {
    const showNotification = () => {
      ...
      if (
        location.state?.donationRegistrationNotification &&
        ...
        api.success({
          ...
          description:
            location.state.donationRegistrationNotification.description,
          placement: "topRight",
          duration: 4,
        });
        ...
        setTimeout(() => {
          navigate(location.pathname, {
            state: {
              ...location.state,
              donationRegistrationNotification: null,
            },
            replace: true,
          });
        }, 100);
      ...
      // Fallback: Check sessionStorage if state is not available
        ...
        sessionStorage.getItem("showDonationSuccessNotification") === "true" &&
        !notificationShown.current
      ) {
        notificationShown.current = true;
        ...
        api.success({
          ...
          description:
            "Cảm ơn bạn đã đăng ký hiến máu, vui lòng kiểm tra email",
          placement: "topRight",
          duration: 4,
        });
  ...
  const handleDownloadCertificate = async (registrationId) => {
    try {
      ...
      const response = await UserAPI.getCertificateByRegistrationId(
        registrationId
      );
  ...
  const handleViewDonationDetail = async (registrationId) => {
    try {
      const donationRecord = await getDonationRecordByRegistrationId(
        registrationId
      );
  ...
  const handleOpenFeedback = async (registrationId) => {
    ...
    console.log(
      `Opening feedback for registration ${registrationId}: feedbackExists = ${feedbackExists}`
    );
    ...
    try {
      ...
      if (hasValidFeedback) {
        ...
        if (
          ...
          console.log(
            "Feedback data exists but has no content, showing submit modal"
          );
    ...
    try {
      ...
      if (error.response && error.response.status === 409) {
        Modal.error({
          ...
          content:
            "Bạn đã gửi phản hồi cho đăng ký này rồi. Mỗi đăng ký chỉ có thể gửi phản hồi một lần duy nhất.",
          okText: "Đã hiểu",
        });
        ...
        api.error({
          ...
          description:
            "Thông tin phản hồi không hợp lệ. Vui lòng kiểm tra lại.",
          placement: "topRight",
          duration: 3,
        });
    ...
    <Layout>
                    ...
                    <div className="donation-stat">
                      ...
                      <div className="stat-content">
                        <div className="stat-label">
                          Thời gian có thể hiến máu lại
                        </div>
                        <div className="stat-value-with-tag">
                          <span className="stat-value">
                            {donationInfo?.nextEligibleDonationDate ||
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          render: (_, record) => {
                            ...
                            const status = registrationStatuses.find(
                              (s) => s.id === statusId
                            );
                            const isCompleted =
                              statusId === 3 ||
                              ...
                              return (
                                <Space direction="vertical" size="small">
                                  <Button
                                    ...
                                    size="small"
                                    onClick={() =>
                                      handleViewDonationDetail(registrationId)
                                    }
                                  >
                                    Xem chi tiết
                                  </Button>
                          ...
                          render: (_, record) => {
                            ...
                            if (isCompleted) {
                              const feedbackExists =
                                feedbackExistence[registrationId];
                              const buttonText = feedbackExists
                                ? "Xem phản hồi"
                                : "Gửi phản hồi";
                              const buttonIcon = feedbackExists ? (
                                <EyeOutlined />
                              ) : (
                                <CommentOutlined />
                              );
                              ...
                              return (
                                <Button
                                  ...
                                  icon={buttonIcon}
                                  onClick={() =>
                                    handleOpenFeedback(registrationId)
                                  }
                                  style={{
                                    color: feedbackExists
                                      ? "#52c41a"
                                      : "#1890ff",
                                    borderColor: feedbackExists
                                      ? "#52c41a"
                                      : "#1890ff",
                                  }}
                                >
            ...
            <Row gutter={[24, 16]}>
              <Col span={8}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {selectedDonationRecord.donorTemperature ||
                      selectedDonationRecord.DonorTemperature ||
                      "N/A"}
                    {selectedDonationRecord.donorTemperature ||
                    selectedDonationRecord.DonorTemperature
              ...
              <Col span={8}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {formatBloodPressure(
                      selectedDonationRecord.donorBloodPressure ||
                        selectedDonationRecord.DonorBloodPressure
                    )}
                  </div>
                </div>
              ...
              <Col span={8}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {selectedDonationRecord.donorWeight ||
                      selectedDonationRecord.DonorWeight ||
                      "N/A"}
                    {selectedDonationRecord.donorWeight ||
                    selectedDonationRecord.DonorWeight
            ...
            <Row gutter={[24, 16]}>
              <Col span={12}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {selectedDonationRecord.donorHeight ||
                      selectedDonationRecord.DonorHeight ||
                      "N/A"}
                    {selectedDonationRecord.donorHeight ||
                    selectedDonationRecord.DonorHeight
              ...
              <Col span={12}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {selectedDonationRecord.donorHeartRate ||
                      selectedDonationRecord.DonorHeartRate ||
                      "N/A"}
                    {selectedDonationRecord.donorHeartRate ||
                    selectedDonationRecord.DonorHeartRate
            ...
            <Row gutter={[24, 16]}>
              <Col span={12}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {getDonationTypeName(
                      selectedDonationRecord.donationTypeId ||
                        selectedDonationRecord.DonationTypeId
                    )}
                  </div>
                </div>
              ...
              <Col span={12}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {selectedDonationRecord.volumeDonated ||
                      selectedDonationRecord.VolumeDonated ||
                      "N/A"}
                    {selectedDonationRecord.volumeDonated ||
                    selectedDonationRecord.VolumeDonated
            ...
            <Row gutter={[24, 16]}>
              <Col span={12}>
                <div className="form-field">
                  ...
                  <div className="form-value">
                    {getBloodTestResultTag(
                      selectedDonationRecord.bloodTestResult ||
                        selectedDonationRecord.BloodTestResult
                    )}
                  </div>
                </div>
      ...
      >
        <div style={{ padding: "16px 0" }}>
          ...
          >
            Trải nghiệm hiến máu của bạn như thế nào? Hãy chia sẻ để giúp chúng
            tôi cải thiện dịch vụ tốt hơn.
          </Text>
          <Input.TextArea
            value={feedbackText}
