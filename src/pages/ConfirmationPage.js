...
const ConfirmationPage = () => {
  ...
  useEffect(() => {
    ...
    if (!bookingData || !eligibilityData) {
      message.warning(
        "Không tìm thấy thông tin đăng ký. Vui lòng bắt đầu lại từ trang đặt lịch."
      );
      navigate("/booking");
      return;
    }
  ...
  const getTimeSlotInfo = (timeSlotId) => {
    ...
    if (timeSlot) {
      return `${timeSlot.startTime.slice(0, 5)} - ${timeSlot.endTime.slice(
        0,
        5
      )}`;
    }
  ...
  const handleConfirmRegistration = async () => {
    ...
    if (!validationResults.isEligible) {
      ...
      if (validationResults.validationType === "warning") {
        ...
        errorMessage =
          'Vui lòng kiểm tra lại các thông tin trong phần "Kết quả đánh giá" và liên hệ với chúng tôi để được tư vấn thêm.';
      }
    ...
    try {
      ...
      let errorMessage =
        "Đã có lỗi xảy ra khi đăng ký hiến máu. Vui lòng thử lại.";
  ...
  const getValidationResults = () => {
    ...
    const userEligibleDate =
      eligibilityData.userEligibleDate ||
      eligibilityData.nextEligibleDonationDate;
    ...
    const hasExistingRegistrations =
      eligibilityData.hasExistingRegistrations || false;
    ...
    if (!formEligible) {
      ...
      if (eligibilityData.last6Months?.includes("weight_loss")) {
        failedChecks.push(
          "Sút cân nhanh không rõ nguyên nhân trong 6 tháng qua"
        );
      }
      ...
      if (eligibilityData.last6Months?.includes("hepatitis_contact")) {
        failedChecks.push(
          "Sinh sống chung với người nhiễm viêm gan B trong 6 tháng qua"
        );
      }

      if (eligibilityData.last6Months?.includes("sexual_contact_risk")) {
        failedChecks.push(
          "Quan hệ tình dục với người có nguy cơ cao trong 6 tháng qua"
        );
      }
      ...
      if (eligibilityData.last7Days?.includes("medication")) {
        failedChecks.push(
          "Sử dụng thuốc kháng sinh, kháng viêm trong 7 ngày qua"
        );
      }
    ...
    if (!isEligibleByDate && daysLeft > 0) {
      ...
      failedChecks.push(
        "Để đảm bảo sức khỏe, bạn cần nghỉ ngơi ít nhất 12-16 tuần giữa các lần hiến máu"
      );
      failedChecks.push(
        `Bạn có thể hiến máu trở lại sau: ${daysLeft} ngày nữa`
      );
      if (userEligibleDate) {
        failedChecks.push(
          `Ngày có thể hiến máu tiếp theo: ${new Date(
            userEligibleDate
        ...
        >
          ...
          <div style={{ textAlign: "center", marginBottom: "40px" }}>
            ...
            <Text style={{ fontSize: "16px", color: "#6b7280" }}>
              Vui lòng kiểm tra lại thông tin của bạn trước khi xác nhận đăng ký
              hiến máu
            </Text>
          </div>
                      ...
                      {validationResults.validationType === "success" ? (
                        ...
                        <div>
                          ...
                          <div style={{ marginBottom: "16px" }}>
                            ...
                            >
                              {validationResults.failedChecks.map(
                                (check, index) => (
                                  <div
                                    key={index}
                                    style={{
                                      ...
                                      marginBottom:
                                        index <
                                        validationResults.failedChecks.length -
                                          1
                                          ? "8px"
                                          : "0",
                                      fontSize: "13px",
